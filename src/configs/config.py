...
class Config:
    ...
    audio_adapter_hidden_dim: int = 1024
    audio_adapter_dropout: float = 0.0
    ...
    t_eps: float = 5e-2
    time_schedule: str = "logit_normal"  # 'logit_normal' or 'uniform'
    xpred_mse_weight: float = 0.0  # Auxiliary weight for denoiser x_pred -> teacher latent MSE.
    xpred_cos_weight: float = 0.0  # Auxiliary weight for denoiser x_pred -> teacher latent cosine loss.
    trajectory_mse_weight: float = 0.0  # Auxiliary weight for short sampling-trajectory latent MSE.
    trajectory_cos_weight: float = 0.0  # Auxiliary weight for short sampling-trajectory cosine loss.
    trajectory_steps: int = 2  # Number of Euler unroll steps used by trajectory auxiliary loss.
    trajectory_start_t: float = 0.0
    trajectory_end_t: float = 1.0
    trajectory_cfg_scale: float = 1.0
    trajectory_decoder_ce_weight: float = 0.0  # Auxiliary CE on stop-grad trajectory sampled latents.
    ...
    eos_token_id: int = None  # Filled from tokenizer at runtime unless explicitly set.
    eos_loss_weight: float = 1.0  # Extra CE weight for EOS target tokens.
    ...
    freeze_elf: bool = False  # If true, train only auxiliary params such as the audio adapter.
    freeze_audio_adapter: bool = False  # If true, keep the audio adapter fixed.
    ...
    init_checkpoint: str = None  # Optional model checkpoint used only to initialize weights.
    resume: str = None
