...
def _pad_or_truncate_feature(feature, target_len):
    """Pad/truncate frame features to a fixed frame count."""
    feature = np.asarray(feature, dtype=np.float32)
    if feature.ndim != 2:
        raise ValueError(f"Expected audio feature with shape (frames, dim), got {feature.shape}")
    length = min(feature.shape[0], target_len)
    feature = feature[:target_len]
    if length < target_len:
        pad = np.zeros((target_len - length, feature.shape[-1]), dtype=np.float32)
        feature = np.concatenate([feature, pad], axis=0)
    return feature, length


def _load_audio_feature(item, audio_feature_key, audio_num_prefix_tokens):
    """Load precomputed audio features from an in-memory value or .npy path."""
    if audio_feature_key in item:
        feature = item[audio_feature_key]
    elif "audio_feature_path" in item:
        feature = np.load(item["audio_feature_path"])
    else:
        raise KeyError(
            f"Audio conditioning is enabled but batch item has neither "
            f"{audio_feature_key!r} nor 'audio_feature_path'."
        )
    return _pad_or_truncate_feature(feature, audio_num_prefix_tokens)
...
def get_dataloader(
    ...
    audio_num_prefix_tokens: int = 32,
):
    """Create a DataLoader."""
    ...
    def collate_fn(batch_list):
        ...
        text_target_len = (
            max_seq_length - audio_num_prefix_tokens if audio_conditioning else max_seq_length
        )
        ids, total_lens = pad_and_truncate(seq_list, text_target_len, pad_token_id)
        pos = np.arange(text_target_len)[None, :]
        ...
        if audio_conditioning:
            ...
            for item in batch_list:
                feature, feature_len = _load_audio_feature(
                    item, audio_feature_key, audio_num_prefix_tokens,
                )
                features.append(feature)
                feature_lens.append(feature_len)
            result["audio_features"] = np.stack(features).astype(np.float32)
            result["audio_feature_mask"] = (
                np.arange(audio_num_prefix_tokens)[None, :] < np.array(feature_lens)[:, None]
            ).astype(np.float32)
...
def load_audiocaps_csv_dataset(path, tokenizer, audio_dir, audio_feature_dir=None, limit=None):
    """Load an AudioCaps-style CSV split.

    The returned examples are caption-generation samples. If audio_feature_dir is
    provided, each example points to the expected precomputed .npy feature file.
    """
    examples = []
    with open(path, "r", encoding="utf-8", newline="") as f:
        ...
        for i, row in enumerate(reader):
            ...
            if not caption:
                continue
            stem = f"{youtube_id}_{start_time}"
            audio_path = os.path.join(audio_dir, f"{stem}.wav")
            if not os.path.exists(audio_path):
                continue
            input_ids = tokenizer(caption, add_special_tokens=False)["input_ids"]
            ...
            if audio_feature_dir:
                example["audio_feature_path"] = os.path.join(audio_feature_dir, f"{stem}.npy")
...
def load_dataset(config, dataset_cache_dir=None):
    ...
    if getattr(config, "audio_conditioning", False) and config.data_path and config.data_path.endswith(".csv"):
        from transformers import AutoTokenizer

        tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_name or config.encoder_model_name)
        audio_dir = os.path.join(os.path.dirname(config.data_path), "audiocaps_raw_audio")
        train_dataset = load_audiocaps_csv_dataset(
            config.data_path, tokenizer, audio_dir, config.audio_feature_dir, config.data_limit,
        )
        log_for_0(f"Loaded AudioCaps train split from {config.data_path}: {len(train_dataset)}")
        eval_dataset = None
        if config.eval_data_path:
            eval_dataset = load_audiocaps_csv_dataset(
                config.eval_data_path, tokenizer, audio_dir, config.audio_feature_dir, config.eval_data_limit,
            )
            log_for_0(f"Loaded AudioCaps eval split from {config.eval_data_path}: {len(eval_dataset)}")
