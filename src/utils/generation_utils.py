...
def _generate_samples_single_batch(
    ...
    cfg_scale: float, self_cond_cfg_scale: float, sample_text_rms_scale: float = 1.0,
) -> Array:
    ...
    def _scale_text_rms(z_in):
        if sample_text_rms_scale == 1.0:
            return z_in
        prefix_len = getattr(config, "audio_num_prefix_tokens", 0)
        text_mask = (jnp.arange(max_length) >= prefix_len)[None, :, None]
        return jnp.where(text_mask, z_in * sample_text_rms_scale, z_in)
    ...
    if method == "sde":
        ...
        def scaled_step_fn(carry, t_pair):
            (z_new, x_new, step_idx), out = step_fn(carry, t_pair)
            return (_scale_text_rms(z_new), x_new, step_idx), out
        (z, x_pred, _), _ = jax.lax.scan(scaled_step_fn, (z, x_pred, jnp.int32(0)), t_pairs)
    else:
        step_fn = _sample_step_for_scan(sampling_config=sampling_config, **step_kwargs)
        def scaled_step_fn(carry, t_pair):
            (z_new, x_new), out = step_fn(carry, t_pair)
            return (_scale_text_rms(z_new), x_new), out
        (z, x_pred), _ = jax.lax.scan(scaled_step_fn, (z, x_pred), t_pairs)

    # Last step always with ode
    z, x_pred = _ode_step(
        z=z, t=t_steps[-2], t_next=t_steps[-1], x_pred_prev=x_pred, **step_kwargs,
    )
    return _scale_text_rms(z)


def _dlm_decode_batch(
    z,
    model_params,
    model_apply_fn,
    t_final_val,
    ...
    if decode_latent_scale != 1.0:
        prefix_len = getattr(config, "audio_num_prefix_tokens", 0)
        text_scale = jnp.full((1, z.shape[1], 1), decode_latent_scale, dtype=z.dtype)
        prefix_mask = (jnp.arange(z.shape[1]) < prefix_len)[None, :, None]
        text_scale = jnp.where(prefix_mask, 1.0, text_scale)
        z = z * text_scale
    ...
    if min_decode_tokens > 0 and eos_token_id >= 0:
        positions = jnp.arange(decoder_logits.shape[1])
        text_positions = positions - getattr(config, "audio_num_prefix_tokens", 0)
        suppress_eos = (text_positions >= 0) & (text_positions < min_decode_tokens)
        special_penalty = jnp.where(suppress_eos, -jnp.inf, 0.0)
        decoder_logits = decoder_logits.at[:, :, eos_token_id].add(special_penalty[None, :])
        if pad_token_id >= 0 and pad_token_id != eos_token_id:
            decoder_logits = decoder_logits.at[:, :, pad_token_id].add(special_penalty[None, :])
