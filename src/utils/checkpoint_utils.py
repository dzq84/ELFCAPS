...
def save_checkpoint(state: Any, output_dir: str, step: int, hf_repo_id: str = None):
    ...
    state_dict = {
        ...
        "audio_adapter_params": state.audio_adapter_params,
        "audio_adapter_opt_state": state.audio_adapter_opt_state,
        "ema_audio_adapter_params1": state.ema_audio_adapter_params1,
...
def _checkpoint_target(state_template: Any):
    return {
        ...
        "audio_adapter_params": state_template.audio_adapter_params,
        "audio_adapter_opt_state": state_template.audio_adapter_opt_state,
        "ema_audio_adapter_params1": state_template.ema_audio_adapter_params1,
...
def load_checkpoint(checkpoint_path: str, state_template: Any) -> Tuple[Any, int]:
    ...
    restored_state = state_template.replace(
        ...
        audio_adapter_params=jax.tree_util.tree_map(
            jnp.array, ckpt.get("audio_adapter_params", state_template.audio_adapter_params),
        ),
        audio_adapter_opt_state=ckpt.get(
            "audio_adapter_opt_state", state_template.audio_adapter_opt_state,
        ),
        ema_audio_adapter_params1=jax.tree_util.tree_map(
            jnp.array,
            ckpt.get(
                "ema_audio_adapter_params1",
                ckpt.get("audio_adapter_params", state_template.ema_audio_adapter_params1),
            ),
        ),
